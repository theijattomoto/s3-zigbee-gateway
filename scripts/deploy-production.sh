...
command -v setfacl >/dev/null 2>&1 || { echo "setfacl is required for secure operator log access. Install the acl package first." >&2; exit 1; }
...
if [ -f "$SOURCE_DIR/PYSerialGateway/README-OPERATOR.md" ]; then
    install -o "$OPERATOR_USER" -g "$OPERATOR_GROUP" -m 644 "$SOURCE_DIR/PYSerialGateway/README-OPERATOR.md" "$OPERATOR_DIR/README-OPERATOR.md"
fi
...
restore_previous_code() {
    ...
    rsync -a --delete --chown=root:s3gw "${RSYNC_EXCLUDES[@]}" "$BACKUP_DIR/" "$TARGET_DIR/" || true
    chmod 750 "$TARGET_DIR/PYSerialGateway/run-service.sh" 2>/dev/null || true
    install -d -o s3gw -g s3gw -m 750 "$TARGET_DIR/PYSerialGateway/log" "$TARGET_DIR/PYSerialGateway/errorlog" 2>/dev/null || true
}

on_exit() {
    exit_code=$?
    trap - EXIT
    if [ "$exit_code" -ne 0 ] && [ "$DEPLOY_STARTED" -eq 1 ]; then restore_previous_code; fi
...
rsync -a --delete --chown=root:s3gw "${RSYNC_EXCLUDES[@]}" "$SOURCE_DIR/" "$TARGET_DIR/"
chmod 750 "$TARGET_DIR/PYSerialGateway/run-service.sh"
install -d -o s3gw -g s3gw -m 750 "$TARGET_DIR/PYSerialGateway/log" "$TARGET_DIR/PYSerialGateway/errorlog"

install -d -o "$OPERATOR_USER" -g s3gw -m 2775 "$OPERATOR_LOG_DIR"
...
migrate_operator_log() {
    ...
    if [ -L "$dest_file" ]; then
        tmp_file="$(mktemp)"; cp -L "$dest_file" "$tmp_file" 2>/dev/null || true; rm -f "$dest_file"
        if [ -s "$tmp_file" ]; then cat "$tmp_file" > "$dest_file"; else : > "$dest_file"; fi
        rm -f "$tmp_file"
    elif [ ! -e "$dest_file" ]; then
        if [ -f "$source_file" ]; then cp "$source_file" "$dest_file"; else : > "$dest_file"; fi
    fi
    chown "$OPERATOR_USER":s3gw "$dest_file"
    chmod 664 "$dest_file"
    setfacl -m u:s3gw:rw "$dest_file"
}

migrate_operator_log "$TARGET_DIR/PYSerialGateway/log/gateway.log" "$OPERATOR_LOG_DIR/gateway.log"
migrate_operator_log "$TARGET_DIR/PYSerialGateway/log/mqtt.log" "$OPERATOR_LOG_DIR/mqtt.log"
migrate_operator_log "$TARGET_DIR/PYSerialGateway/errorlog/error.log" "$OPERATOR_LOG_DIR/error.log"

for log_file in gateway.log mqtt.log error.log; do
    runuser -u s3gw -- test -w "$OPERATOR_LOG_DIR/$log_file" || { echo "Operator log is not writable by s3gw: $OPERATOR_LOG_DIR/$log_file" >&2; exit 1; }
done

set_env_value() {
    key="$1"; value="$2"; env_file="$TARGET_DIR/.env"
...
"$TARGET_DIR/.venv/bin/python" -c "for p in ['$TARGET_DIR/pyserialgateway/PYGatewayListener/config.py','$TARGET_DIR/pyserialgateway/PYGatewayListener/db_connection.py','$TARGET_DIR/pyserialgateway/PYGatewayListener/rest_api.py']: compile(open(p).read(), p, 'exec'); print('SYNTAX_PASS', p)"
...
TARGET_DIR="$TARGET_DIR" OPERATOR_DIR="$OPERATOR_DIR" OPERATOR_USER="$OPERATOR_USER" OPERATOR_GROUP="$OPERATOR_GROUP" OPERATOR_HOME="$OPERATOR_HOME" BACKUP_ROOT="$BACKUP_ROOT" bash "$SOURCE_DIR/scripts/setup-operator-gps-access.sh"

echo "Applying runtime hardening..."
TARGET_DIR="$TARGET_DIR" SERVICE_NAME="$SERVICE_NAME" bash "$SOURCE_DIR/scripts/install-runtime-hardening.sh"

if [ "$(readlink -f "$TARGET_DIR/PYSerialGateway/GPSlog")" != "$(readlink -f "$OPERATOR_DIR/GPSlog")" ]; then echo "GPS operator path validation failed." >&2; exit 1; fi
runuser -u s3gw -- test -w "$OPERATOR_DIR/GPSlog" || { echo "GPS operator directory is not writable by s3gw." >&2; exit 1; }
runuser -u s3gw -- test -w "$TARGET_DIR/pyserialgateway/hardware_reset.py" && { echo "Hardware reset script is unexpectedly writable by s3gw." >&2; exit 1; }
runuser -u s3gw -- test -w "$TARGET_DIR/pyserialgateway/config_PYproperties.py" && { echo "Hardware reset configuration is unexpectedly writable by s3gw." >&2; exit 1; }

resolved_exec="$(systemctl show "$SERVICE_NAME" -p ExecStart --value)"
case "$resolved_exec" in *"run-service.sh GPSUP"*) ;; *) echo "GPSUP is missing from effective systemd ExecStart: $resolved_exec" >&2; exit 1;; esac

echo "Starting $SERVICE_NAME..."
systemctl start "$SERVICE_NAME"
systemctl is-active --quiet "$SERVICE_NAME" || { echo "Service did not become active after deployment." >&2; exit 1; }
ps -eo user,args | grep -F "pygw_main.py GPSUP" | grep -q '^s3gw ' || { echo "Gateway process did not start with GPSUP." >&2; exit 1; }
