...
if [ -e "$RUNTIME_GPS_DIR" ] && [ ! -d "$RUNTIME_GPS_DIR" ]; then
    echo "Refusing to replace unexpected non-directory GPSlog path: $RUNTIME_GPS_DIR" >&2
    exit 1
fi
...
# rsync -a may copy legacy directory ownership/mode onto the destination.
# Reapply the intended operator/service ACLs after migration.
apply_operator_gps_permissions
...
if [ -d "$BACKUP_DIR/GPSlog-legacy" ]; then
